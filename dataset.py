...
def _get_crf_list():
    ...
    # the index of muptiple of 6 : brightness 
...
def _inverse_rf(_rf):  # [s]
    ...
    rf[0] = 0.0
    rf[-1] = 1.0
    return interp1d(rf, np.linspace(0.0, 1.0, num=s))(np.linspace(0.0, 1.0, num=s))
...
class MergeDataset(Dataset):
    ...
    def __getitem__(self, all_idx):
        ...
        for dataset in self._dataset_list:
            ...
            # logging.info(f"all_idx : {all_idx} curr_idx : {curr_idx}  len(dataset) : {len(dataset)}")
...
class PatchHDRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self._is_training:
            ...
            # 256x256 random cropping & rotation
